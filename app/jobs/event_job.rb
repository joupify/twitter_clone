class EventJob < ApplicationJob
  ...
  def perform(event)
    ...
    when 'like'
      # Logique spécifique pour un événement "like"
 ...
 event.mark_as_completed
  ...
  def tweet_notification_event(event)
    ...
    if emails.any? && user.followers.any?
      Rails.logger.info "Envoi d'emails à #{emails.size} followers pour le tweet #{tweet.id}"
      TweetNotifier.with(tweet: tweet, user: user, emails: emails).deliver_later(user.followers)
    end
  end
end
